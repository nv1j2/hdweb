...
<body onselectstart="return false;" >
    <div class="top">
    	...
    	<div class='tright'>
    		...
    		<span class="icon-cguanliyuan nicheng" <c:if test="${wdxxs>0}">style="margin-right:0"</c:if> title="${ses_admin}">${ses_admin}</span>
    ...
    <script type="text/javascript">
			    ...
			    iframe.load(function(){
			        ...
			        var doURL = $(this)[0].contentWindow.document.URL;
 			        $(window.frames["nm_frm_"+lanmu+id].document.body).append("<script>if ('"+doURL+"'.indexOf('${dlhtlj}.dhtml')!=-1"
 			        	+"&& window.top!=null){window.top.location= '${path}/${dlhtlj}.dhtml'}<\/script>");
			        $(".bigload").hide();
			        iframe.show();
			    })
		...
		$(function (){
			...
			var zNodes =[
				{lmlj:'lanmu', id:-1,name:"栏目列表"},
				{lmlj:'yqlj',id:-2, name:"友情链接"},
				{lmlj:'zdb',id:-3, name:"字典表"},
				{name:"配置项",id:-4,children:[{name:'可编辑',lmlj:'cfg_kbj',id:-41},{name:'不可编辑',lmlj:'cfg',id:-42}]},
				{lmlj:'wzjth',id:-5, name:"网站静态化"},
			];
		...
		function fn_adtc(tag){
			$.ajax({
				...
				success:function(data){
					layer.closeAll();
					location.href = "${path}/${dlhtlj}.dhtml";
				},
				error: function(){
